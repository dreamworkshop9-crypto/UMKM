...
<body class="font-sans text-white antialiased" style="background:#0f172a">
    ...
    <nav id="navbar" class="glass-nav fixed top-0 left-0 right-0 z-50 border-b border-slate-700/30 transition-all duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16 md:h-20">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @endguest
                    @auth
                    <a href="{{ route('pelanggan.pesanan') }}" class="hidden sm:inline-flex items-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-lg transition-colors">
                    ...
                    <div class="hidden sm:block relative" id="dd-wrap">
                        ...
                        <div id="dd-menu" class="hidden absolute right-0 top-full mt-2 w-56 bg-slate-800 border border-slate-700/30 rounded-xl shadow-2xl overflow-hidden z-50">
                            ...
                            <div class="py-1">
                                <a href="{{ route('pelanggan.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-300 hover:bg-slate-700/50"><i class="fa-solid fa-gauge w-4 text-center text-slate-500"></i>Dashboard</a>
                                <a href="{{ route('pelanggan.pesanan') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-300 hover:bg-slate-700/50"><i class="fa-solid fa-box w-4 text-center text-slate-500"></i>Pesanan Saya</a>
                            </div>
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-slate-700/30">
            <div class="px-4 py-4 space-y-1">
                ...
                @endguest
                @auth
                <a href="{{ route('pelanggan.pesanan') }}" onclick="toggleMobileMenu()" class="block px-4 py-3 text-sm font-medium text-slate-300 hover:bg-slate-700/50 rounded-lg"><i class="fa-solid fa-box mr-2"></i>Pesanan Saya</a>
    ...
    <section class="py-16 border-t border-slate-700/20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex gap-3 overflow-x-auto scrollbar-hide pb-2" id="category-filter">
                ...
                <button onclick="setCategory('{{ $kat->slug }}')" data-cat="{{ $kat->slug }}" class="cat-btn flex-shrink-0 px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 bg-slate-800 text-slate-400 border border-slate-700/30 hover:border-brand-500/50 hover:text-white">
    ...
    <section id="produk" class="py-16 lg:py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div id="product-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                @endphp
                <div class="product-card bg-dashboard-card rounded-2xl border border-slate-700/30 overflow-hidden card-hover img-zoom group fade-up visible" data-name="{{ $pNama }}" data-kategori="{{ $pSlug }}" data-umkm="{{ $pBrand }}">
                    <div class="relative overflow-hidden cursor-pointer" onclick="openProductModal('{{ $p->id }}')">
                        <img src="{{ $pGambar }}" alt="{{ $pNama }}" class="w-full h-56 object-cover" onerror="this.src='https://picsum.photos/seed/shoe-{{ $p->id }}/400/400.jpg'">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                        @if($pStok <= 5)<span class="absolute top-3 left-3 px-2.5 py-1 bg-rose-500/90 text-white text-[10px] font-bold rounded-lg uppercase tracking-wide">Stok Terbatas</span>@endif
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <button onclick="event.stopPropagation();quickAddToCart('{{ $p->id }}')" class="w-9 h-9 bg-brand-500/10 hover:bg-brand-500 text-brand-400 hover:text-white rounded-lg flex items-center justify-center transition-all duration-200"><i class="fa-solid fa-plus text-sm"></i></button>
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <form id="checkout-form" onsubmit="submitOrder(event)" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        @foreach($metodeBayar ?? ['transfer'=>'Transfer Bank','ewallet'=>'E-Wallet','cod'=>'COD','qris'=>'QRIS'] as $val => $label)
                        <label class="flex items-center gap-3 px-4 py-3 bg-slate-800 border border-slate-600/50 rounded-xl cursor-pointer hover:border-brand-500/50 transition-all has-[:checked]:border-brand-500 has-[:checked]:bg-brand-500/5">
                            <input type="radio" name="pembayaran" value="{{ $val }}" required class="accent-brand-500">
    ...
    <div id="success-modal" class="fixed inset-0 bg-black/70 z-[60] hidden flex items-center justify-center p-4">
        <div class="bg-slate-900 border border-slate-700/30 rounded-2xl w-full max-w-sm text-center p-10">
            ...
            <a href="{{ route('pelanggan.pesanan') }}" class="block w-full py-3.5 bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-400 hover:to-brand-500 text-white font-semibold rounded-xl transition-all mb-3">Lihat Pesanan Saya</a>
...
</html> ,tambahkan fitur transaksi payment gateaway,tambah kerajang 