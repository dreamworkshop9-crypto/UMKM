...
<body class="font-sans text-white antialiased" style="background:#0f172a">
    ...
    <div id="account-sidebar" class="fixed top-0 right-0 bottom-0 w-full max-w-xl bg-slate-900 border-l border-slate-700/30 z-50 transform translate-x-full transition-transform duration-300 flex flex-col">
        ...
        <div class="flex border-b border-slate-800 scrollbar-hide overflow-x-auto flex-shrink-0">
            ...
            <button onclick="switchAccountTab('alamat')" id="btn-acc-alamat" class="flex-1 min-w-[120px] py-3.5 text-sm font-semibold text-center border-b-2 border-transparent text-slate-400 hover:text-white transition-all">Alamat</button>
            <button onclick="switchAccountTab('ulasan')" id="btn-acc-ulasan" class="flex-1 min-w-[120px] py-3.5 text-sm font-semibold text-center border-b-2 border-transparent text-slate-400 hover:text-white transition-all">Ulasan</button>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {{-- Tab 4: Alamat --}}
            <div id="tab-acc-content-alamat" class="space-y-4 hidden">
                <div class="bg-slate-800/40 border border-slate-700/20 rounded-2xl p-5 text-center py-10">
                    <i class="fa-solid fa-map-location-dot text-4xl text-slate-700 mb-3 block"></i>
                    <h5 class="text-sm font-semibold text-white">Daftar Alamat</h5>
                    <p class="text-xs text-slate-500 mt-1 max-w-xs mx-auto">Anda dapat mengelola alamat pengiriman langsung pada saat melakukan pengisian formulir Checkout.</p>
                </div>
            </div>

            {{-- Tab 5: Ulasan --}}
            <div id="tab-acc-content-ulasan" class="space-y-4 hidden">
                <div class="bg-slate-800/40 border border-slate-700/20 rounded-2xl p-5 text-center py-10">
                    <i class="fa-solid fa-star-half-stroke text-4xl text-slate-700 mb-3 block"></i>
                    <h5 class="text-sm font-semibold text-white">Tidak ada ulasan tertunda</h5>
                    <p class="text-xs text-slate-500 mt-1 max-w-xs mx-auto">Semua pesanan yang telah selesai sudah Anda beri ulasan, atau Anda belum memiliki pesanan.</p>
                </div>
            </div>
...
const shippingRates = @json($ongkirRates ?? []);
    ...
    async function quickAddToCart(id, ukuran = null, warna = null) { try { const r = await api(APP.routes.keranjangTambah, 'POST', { produk_id: id, ukuran, warna, qty: 1 }); setCart(r.data); showToast(r.message || 'Ditambahkan', 'success'); } catch (e) { showToast(e.message || 'Gagal', 'error'); if (e.message && e.message.toLowerCase().includes('login')) { setTimeout(() => openAuthModal('login'), 800); } } }
                        ...
                        container.innerHTML = res.data.map(item => `
                            <div class="flex items-center gap-4 bg-slate-800/30 border border-slate-700/20 rounded-2xl p-4 transition-all duration-300 hover:border-slate-700/50">
                                ...
                                <div class="flex items-center gap-2">
                                    <button onclick="quickAddToCart('${item.product_id}')" class="w-8 h-8 rounded-lg bg-brand-500/10 hover:bg-brand-500 text-brand-400 hover:text-white flex items-center justify-center transition-all duration-200" title="Beli Sekarang">
...
async function openProductModal(id) {
    ...
    function addToCartModal(id) { quickAddToCart(id, document.getElementById('sel-size').value, document.getElementById('sel-color').value); closeProductModal(); }
