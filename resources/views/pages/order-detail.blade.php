        ...
        <div class="p-6 sm:px-10 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-950/50">
            <div>
                ...
                @if($order->status == 'pending')
                    <span class="px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-bold uppercase tracking-widest inline-flex items-center gap-2"><i class="fa-solid fa-clock"></i> Menunggu Pembayaran</span>
                @elseif($order->status == 'processing')
                    <span class="px-4 py-2 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-bold uppercase tracking-widest inline-flex items-center gap-2"><i class="fa-solid fa-spinner fa-spin"></i> Sedang Diproses</span>
                @elseif($order->status == 'shipped')
                    <span class="px-4 py-2 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-bold uppercase tracking-widest inline-flex items-center gap-2"><i class="fa-solid fa-truck-fast"></i> Sedang Dikirim</span>
                ...
                @else
                    <span class="px-4 py-2 rounded-xl bg-slate-500/10 border border-slate-500/20 text-slate-400 text-xs font-bold uppercase tracking-widest">{{ $order->status_label }}</span>
                @endif
            </div>
        ...
        <div class="p-6 sm:p-10 border-b border-slate-800">
            ...
            <div class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                    <!-- Product Image -->
                    <div class="w-20 h-20 rounded-xl bg-slate-800/50 p-2 flex-shrink-0 border border-slate-700/50 flex items-center justify-center">
                        <img src="{{ $item->product->thumbnail_url }}" alt="{{ $item->product->name }}" class="max-w-full max-h-full object-contain"/>
                    </div>
                    
                    <!-- Product Info -->
                    <div class="flex-1">
                        <h4 class="text-base font-bold text-white leading-tight mb-1">{{ $item->product->name }}</h4>
                    ...
                    <div class="text-right w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t border-slate-800 sm:border-0">
                        ...
                        <div class="text-lg font-bold text-white">Rp{{ number_format($item->subtotal,0,',','.') }}</div>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div>
                ...
                <div class="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <div class="text-sm font-bold text-white">{{ $order->name }}</div>
                            <div class="text-sm text-slate-400">{{ $order->phone }}</div>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <i class="fa-solid fa-location-dot text-slate-500 mt-1"></i>
                        <div>
                            <div class="text-sm text-slate-300 leading-relaxed">{{ $order->address }}</div>
                            <div class="text-sm font-semibold text-slate-400 mt-1">{{ $order->city }}, {{ $order->postal_code }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="bg-slate-950 border border-slate-800 rounded-2xl p-5">
                    ...
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-sm font-medium text-white">{{ $order->total_formatted }}</span>
                    </div>
                    ...
                    <div class="flex justify-between items-end pt-4 border-t border-slate-800">
                        ...
                        <span class="text-2xl font-bold text-emerald-400">{{ $order->total_formatted }}</span>
                    </div>
                </div>
            </div>
