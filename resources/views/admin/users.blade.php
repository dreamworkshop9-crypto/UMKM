...
<div class="max-w-[1200px] mx-auto">
    ...
    <div class="flex items-center gap-3 mb-4">
        <div class="relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau email..." class="search-input" id="searchInput" autocomplete="off"/>
        </div>
    ...
    <div class="tbl-wrapper">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-slate-500">{{ ($users->currentPage() - 1) * $users->perPage() + $i + 1 }}</td>
                    <td>
                    ...
                    <td>
                        <span class="badge-status badge-{{ $user->email_verified_at ? 'active' : 'inactive' }}">
                            <span class="dot"></span>
                            {{ $user->email_verified_at ? 'Aktif' : 'Nonaktif' }}
                        </span>
                    </td>
                    <td class="text-slate-500">{{ $user->created_at ? $user->created_at->format('d M Y') : '-' }}</td>
                    <td>
                        <div class="flex items-center justify-center gap-2">
                            <button type="button" class="btn-action btn-view" title="Detail" onclick="viewUser({{ $user->id }})">
                                <span class="material-symbols-outlined" style="font-size:18px">visibility</span>
                            </button>
                            <button type="button" class="btn-action btn-edit" title="Edit" onclick="openEdit({{ $user->id }})">
                                <span class="material-symbols-outlined" style="font-size:18px">edit</span>
                            </button>
                            <button type="button" class="btn-action btn-delete" title="Hapus" onclick="confirmDelete({{ $user->id }}, '{{ $user->name }}')">
        ...
        <div class="flex items-center justify-between px-5 py-4 border-t border-outline-variant/20">
            <p class="text-[12px] text-slate-500">Menampilkan {{ $users->firstItem() }}-{{ $users->lastItem() }} dari {{ $users->total() }} data</p>
            <div class="pagination">{{ $users->withQueryString()->links('admin.partials.pagination-custom') }}</div>
        </div>
        @endif
    </div>
</div>
