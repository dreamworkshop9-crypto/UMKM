...
@endsection
@endsection
cat > resources/views/components/admin-sidebar.blade.php << 'ENDOFFILE'
<aside class="w-64 bg-dashboard-card flex-shrink-0 flex flex-col border-r border-slate-700/50 fixed h-full z-30 shadow-sm">
    <div class="p-6 flex items-center space-x-2">
        <div class="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center shadow-md shadow-md shadow-purple-500/25">
    ...
    <nav class="flex-1 overflow-y-auto px-4 py-2 text-sm">
        <div class="mb-6">
            <p class="px-4 text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2">Menu Utama</p>
            <a class="sidebar-link @if(Route::currentRouteName()==='admin.dashboard') active @else text-slate-400 hover:bg-slate-700/50 hover:text-white @endif flex items-center px-4 py-2.5 rounded-lg transition-all" href="{{ route('admin.dashboard') }}">
                <svg class="w-5 h-5 mr-3 text-slate-400"><i class="fa-solid fa-chart-pie w-5 text-center"></svg></svg>Dashboard
            </a>
            <a class="sidebar-link @if(Route::currentRouteName()==='admin.brands.index') active @else text-slate-400 hover:bg-slate-700/50 hover:text-white @endif flex items-center px-4 py-2.5 rounded-lg transition-all" href="{{ route('admin.brands.index') }}">
                <svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-tag w-4 h-4"></svg></svg>Merek
            </a>
            <a class="sidebar-link @if(Route::currentRouteName()==='admin.kategori') active @else text-slate-400 hover:bg-slate-700/50 hover:text-white @endif flex items-center px-4 py-2.5 rounded-lg transition-all" href="{{ route('admin.kategori') }}">
                <svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-layer-group w-4 h-4"></svg></svg>Kategori
            </a>
            <li>
                <div class="flex items-center px-4 py-2 rounded-lg text-slate-400 cursor-default">
                    <svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-shoe-1/2 h-4"></i class="fa-solid fa-shoe-1/2 w-4 h-4"></svg>Produk
                </div>
                <ul class="ml-7 mt-1 space-y-1 border-l border-slate-700/50 pl-4">
                    <li><a class="flex items-center px-3 py-1.5 rounded-lg transition-colors @if(Route::currentRouteName()==='admin.produk') bg-slate-700/50 text-white font-medium @else text-[13px] text-slate-500 hover:bg-slate-700/50 hover:text-white @endif" href="{{ route('admin.produk') }}">
                        <span class="w-1.5 h-1.5 rounded-full @if(Route::currentRouteName()==='admin.produk') bg-purple-400 @else bg-slate-600 @endif mr-2.5 flex-shrink-0"></span>Data Produk
                    </a></li>
                    <li><a class="flex items-center px-3 py-1.5 rounded-lg transition-colors @if(Route::currentRouteName()==='admin.produk.create') bg-slate-700/50 text-white font-medium @else text-[13px] text-slate-500 hover:bg-slate-700/50 hover:text-white @endif" href="{{ route('admin.produk.create') }}">
                        <span class="w-1.5 h-1.5 rounded-full @if(Route::currentRouteName()==='admin.produk.create') bg-purple-400 @else bg-slate-600 @endif mr-2.5 flex-shrink-0"></span>Tambah Produk
                    </a></li>
                </ul>
            </li>
            <li><a class="flex items-center justify-between px-4 py-2.5 rounded-lg transition-colors text-slate-400 hover:bg-slate-700/50 hover:text-white" href="{{ route('admin.slider') }}">
                <span class="flex items-center"><svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-image w-4 h-4"></svg></svg>Slider
            </span>
            </a>
            <li><a class="flex items-center justify-between px-4 py-2.5 rounded-lg transition-colors text-slate-400 hover:bg-slate-700/50 hover:text-white" href="{{ route('admin.kupon') }}">
                <span class="flex items-center"><svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-ticket w-4 h-4"></svg></svg>Kupon
            </span>
            </a></li>
            <li><a class="flex items-center justify-between px-4 py-2.5 rounded-lg transition-colors text-slate-400 hover:bg-slate-700/50 hover:text-white" href="{{ route('admin.wilayah') }}">
                <span class="flex items-center"><svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-map-location-dot w-4 h-4"></svg></svg>Data Wilayah
            </span>
            </a></li>
            </ul>
        </div>
        ...
        <div class="mb-6">
            ...
            <ul class="ml-7 mt-1 space-y-1 border-l border-slate-700/50 pl-4">
                <li>
                    <a class="flex items-center px-3 py-1.5 rounded-lg transition-colors text-[13px] @if(Route::currentRouteName()=='admin.pesanan') bg-slate-700/50 text-white font-medium @else text-slate-500 hover:bg-slate-700/50 hover:text-white @endif" href="{{ route('admin.pesanan') }}">
                        <span class="w-1.5 h-1.5 rounded-full bg-purple-400 mr-2.5 flex-shrink-0"></span>Data Pesanan
                    </a>
                </li>
                <li>
                    <a class="flex items-center px-3 py-1.5 rounded-lg transition-colors text-[13px] @if(Route::currentRouteName()==='admin.pesanan.konfirmasi') bg-sky-700/50 text-sky-400 font-medium @else text-slate-500 hover:bg-sky-700/50 hover:text-white @endif" href="{{ route('admin.pesanan.konfirmasi') }}">
                        <span class="w-1.5 h-1.5 rounded-full @if(Route::currentRouteName()=='admin.pesanan.konfirmasi') bg-sky-400 @else bg-slate-600 @endif mr-2.5 flex-shrink-0"></span>Konfirmasi
                    </a>
                </li>
                <li>
                    <a class="flex items-center px-3 py-1.5 rounded-lg transition-colors text-[13px] @if(Route::currentRouteName()=='admin.pesanan.dalam-perjalanan') bg-amber-500/50 text-amber-400 font-medium @else text-slate-500 hover:bg-slate-700/50 hover:text-white @endif" href="{{ route('admin.pesanan.dalam-perjalanan') }}">
                        <span class="w-1.5 h-1.5 rounded-full @if(Route::currentRouteName()=='admin.pesanan.dalam-perjalangan') bg-amber-400 @else bg-slate-600 @endif mr-2.5 flex-shrink-0"></span>Dalam Perjalanan
                    </a>
                </li>
                <li>
                    <a class="flex items-center px-3 py-1.5 rounded-lg transition-colors text-[13px] @if(Route::currentRouteName()=='admin.pesanan.dikemas') bg-purple-500/50 text-purple-400 font-medium @else text-slate-500 hover:bg-slate-700/50 hover:text-white @endif" href="{{ route('admin.pesanan.dikemas') }}">
                        <span class="w-1.5 h-1.5 rounded-full @if(Route::currentRouteName()=='admin.pesanan.dikemas') bg-purple-400 @else bg-slate-600 @endif mr-2.5 flex-shrink-0"></span>Dikemas
                    </a>
                </li>
                <li>
                    <a class="flex items-center px-3 py-1.5 rounded-lg transition-colors text-[13px] @if(Route::currentRouteName()=='admin.pesanan.dikirim') bg-cyan-500/50 text-cyan-400 font-medium @else text-slate-500 hover:bg-slate-700/50 hover:text-white @endif" href="{{ route('admin.pesanan.dikirim') }}">
                        <span class="w-1.5 h-1.5 rounded-full @if(Route::currentRouteName()=='admin.pesanan.dikirim') bg-cyan-400 @else bg-slate-600 @endif mr-2.5 flex-shrink-0"></span>Dikirim
                    </a>
                </li>
                <li>
                    <a class="flex items-center px-3 py-1.5 rounded-lg transition-colors text-[13px] @if(Route::currentRouteName()=='admin.pesanan.selesai') bg-emerald-500/50 text-emerald-400 font-medium @else text-slate-500 hover:bg-slate-700/50 hover:text-white @endif" href="{{ route('admin.pesanan.selesai') ?>">
                        <span class="w-1.5 h-1.5 rounded-full @if(Route::currentRouteName()=='admin.pesanan.selesai') bg-emerald-400 @else bg-slate-600 @endif mr-2.5 flex-shrink-0"></span>Selesai
                    </a>
                </li>
            </ul>
        </div>
        ...
        <div class="mb-6">
            ...
            <ul class="space-y-1">
                <li><a class="flex items-center justify-between px-4 py-2.5 rounded-lg transition-colors text-slate-400 hover:bg-slate-700/50 hover:text-white" href="{{ route('admin.users') }}"><span class="flex items-center"><svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-user w-4 h-4"></svg></svg>Data User</span></a></li>
                <li><a class="flex items-center justify-between px-4 py-2.5 rounded-lg transition-colors text-slate-400 hover:bg-slate-700/50 hover:text-white" href="{{ route('admin.admins') }}"><span class="flex items-center"><svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-user-shield w-4 h-4"></svg></svg>Data Admin</span></a></li>
                <li><a class="flex items-center justify-between px-4 py-2.5 rounded-lg transition-colors text-slate-400 hover:bg-slate-700/50 hover:text-white" href="{{ route('admin.laporan') }}"><span class="flex items-center"><svg class="w-4 h-4 mr-3 text-slate-400"><i class="fa-solid fa-chart-pie w-4 h-4"></svg></svg>Laporan</span></a></li>
            </ul>
        </div>
    </nav>
</aside>
