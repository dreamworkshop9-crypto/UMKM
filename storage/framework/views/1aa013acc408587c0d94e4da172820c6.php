<?php $__env->startSection('title','Detail Pesanan - SALZA'); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-4xl mx-auto px-6 py-12">
    <!-- Header Area -->
    <div class="mb-8">
        <a href="<?php echo e(route('order.index')); ?>" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-400 hover:text-purple-400 transition-colors mb-4">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-400"><?php echo e($order->created_at->format('d F Y, H:i')); ?></p>
                </div>
            </div>
            
            <div class="flex flex-col items-end gap-1">
                <span class="text-[10px] uppercase tracking-widest font-bold text-slate-500">Invoice</span>
                <span class="text-lg font-bold text-white bg-slate-900 px-4 py-1.5 rounded-lg border border-slate-700"><?php echo e($order->invoice); ?></span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div class="p-6 sm:px-10 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-950/50">
            <div>
                ...
                <?php elseif($order->status == 'diperjalanan'): ?>
                ...
                <?php else: ?>
                    <span class="px-4 py-2 rounded-xl bg-slate-500/10 border border-slate-500/20 text-slate-400 text-xs font-bold uppercase tracking-widest"><?php echo e(ucfirst($order->status)); ?></span>
                <?php endif; ?>
            </div>
    ...
    <img src="<?php echo e($item->product->image_url ?? asset('images/default-product.png')); ?>" alt="<?php echo e($item->product->name ?? 'Produk'); ?>" class="max-w-full max-h-full object-contain"/>
</div>
                    
                    <!-- Product Info -->
                    <div class="flex-1">
                        <h4 class="text-base font-bold text-white leading-tight mb-1"><?php echo e($item->product->name ?? 'Produk Tidak Tersedia'); ?></h4>
                        <div class="flex flex-wrap gap-2 mb-2">
                            <?php if($item->size): ?>
                                <span class="text-xs text-slate-400 bg-slate-800 px-2 py-0.5 rounded border border-slate-700">Ukuran: <?php echo e($item->size); ?></span>
                            <?php endif; ?>
                            <?php if($item->color && $item->color !== '-'): ?>
                                <span class="text-xs text-slate-400 bg-slate-800 px-2 py-0.5 rounded border border-slate-700">Warna: <?php echo e($item->color); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="text-sm font-semibold text-emerald-400"><?php echo e($item->quantity); ?> × Rp<?php echo e(number_format($item->price, 0, ',', '.')); ?></div>
                    </div>
                    ...
                    <div class="text-right w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t border-slate-800 sm:border-0">
                        ...
                        <div class="text-lg font-bold text-white">Rp<?php echo e(number_format($item->price * $item->quantity, 0, ',', '.')); ?></div>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <div class="text-sm font-bold text-white"><?php echo e($order->shipping_name ?? '-'); ?></div>
                            <div class="text-sm text-slate-400"><?php echo e($order->shipping_phone ?? '-'); ?></div>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <i class="fa-solid fa-location-dot text-slate-500 mt-1"></i>
                        <div>
                            <div class="text-sm text-slate-300 leading-relaxed"><?php echo e($order->shipping_address ?? '-'); ?></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>   
                ...
                <div class="bg-slate-950 border border-slate-800 rounded-2xl p-5">
                    <div class="flex justify-between items-center mb-4 pb-4 border-b border-slate-800">
                        ...
                        <span class="text-sm font-bold text-white bg-slate-800 px-3 py-1 rounded-lg border border-slate-700"><?php echo e(strtoupper($order->payment_method)); ?></span>
                    </div>
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-sm text-slate-400">Subtotal Produk</span>
                        <span class="text-sm font-medium text-white">Rp<?php echo e(number_format($order->total, 0, ',', '.')); ?></span>
                    </div>
                    ...
                    <div class="flex justify-between items-end pt-4 border-t border-slate-800">
                        ...
                        <span class="text-2xl font-bold text-emerald-400">Rp<?php echo e(number_format($order->total, 0, ',', '.')); ?></span>
                    </div>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/irawan/laravel/shoesmarket/UMKMmm/UMKM/resources/views/pages/order-detail.blade.php ENDPATH**/ ?>