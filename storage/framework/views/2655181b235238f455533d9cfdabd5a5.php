...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'SALZA - Toko Sepatu Online'); ?></title>
...
<body class="flex flex-col min-h-screen selection:bg-purple-500/30 selection:text-purple-200">
        ...
        <div class="max-w-7xl mx-auto px-6 flex justify-between items-center text-[11px] font-medium text-slate-400 tracking-wider uppercase">
            ...
            <div class="flex items-center gap-4">
                <?php if(auth()->guard()->check()): ?>
                    <a href="<?php echo e(route('wishlist.index')); ?>" class="hover:text-purple-400 transition-colors"><i class="fa-regular fa-heart mr-1"></i> Wishlist</a>
                    <a href="<?php echo e(route('order.index')); ?>" class="hover:text-emerald-400 transition-colors"><i class="fa-solid fa-box-open mr-1"></i> Pesanan</a>
                    ...
                    <form action="<?php echo e(route('logout')); ?>" method="POST" class="inline"><?php echo csrf_field(); ?>
                        <button type="submit" class="hover:text-rose-400 transition-colors"><i class="fa-solid fa-arrow-right-from-bracket mr-1"></i> Keluar (<?php echo e(Auth::user()->name); ?>)</button>
                    </form>
                <?php else: ?>
                    ...
                    <div class="w-px h-3 bg-slate-700"></div>
                    <a href="<?php echo e(route('login')); ?>" class="hover:text-white transition-colors">Masuk</a>
                    <a href="<?php echo e(route('daftar')); ?>" class="hover:text-purple-400 transition-colors">Daftar</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <header class="glass-nav sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex items-center justify-between h-20 gap-8">
                <!-- Logo -->
                <a href="<?php echo e(route('home')); ?>" class="text-2xl font-bold tracking-tighter text-white flex-shrink-0">
                    SAL<span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-emerald-400">ZA</span>
                </a>

                <!-- Navigation -->
                <nav class="hidden lg:flex items-center gap-8">
                    <a href="<?php echo e(route('home')); ?>" class="nav-link text-sm font-semibold text-slate-300 hover:text-white <?php echo e(request()->routeIs('home') ? 'active text-white' : ''); ?>">Beranda</a>
                    
                    <?php $__currentLoopData = $categories ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="relative group py-8">
                        <a href="<?php echo e(route('shop',['category'=>$cat->slug])); ?>" class="nav-link text-sm font-semibold text-slate-300 hover:text-white flex items-center gap-1.5">
                            <?php echo e($cat->name); ?> <i class="fa-solid fa-chevron-down text-[10px] text-slate-500 group-hover:text-purple-400 transition-colors"></i>
                        </a>
                        ...
                        <div class="dropdown-menu absolute top-[70px] left-0 w-64 bg-slate-800 border border-slate-700/50 rounded-2xl shadow-2xl shadow-black/50 opacity-0 invisible translate-y-2 transition-all duration-300 overflow-hidden">
                            ...
                            <div class="p-4 border-b border-slate-700/50 last:border-0">
                                <h4 class="text-[11px] font-bold uppercase tracking-wider text-purple-400 mb-2 px-2"><?php echo e($sub->name); ?></h4>
                                <div class="flex flex-col">
                                    <?php $__currentLoopData = $sub->children; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ss): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <a href="<?php echo e(route('product.subsubcategory',[$ss->id,$ss->slug])); ?>" class="px-2 py-1.5 text-sm text-slate-300 hover:text-white hover:bg-slate-700/50 rounded-lg transition-colors"><?php echo e($ss->name); ?></a>
                    ...
                    <a href="<?php echo e(route('shop')); ?>" class="nav-link text-sm font-semibold text-slate-300 hover:text-white <?php echo e(request()->routeIs('shop') ? 'active text-white' : ''); ?>">Belanja</a>
                    <a href="<?php echo e(route('shop',['sort'=>'price_asc'])); ?>" class="text-sm font-bold text-amber-400 hover:text-amber-300 transition-colors">PROMO 🔥</a>
                </nav>

                <!-- Search & Actions -->
                <div class="flex items-center gap-4 flex-1 justify-end">
                    <!-- Search Bar -->
                    <form action="<?php echo e(route('shop')); ?>" method="GET" class="hidden md:flex relative w-full max-w-xs group">
                        <input type="text" name="q" placeholder="Cari sepatu impianmu..." value="<?php echo e(request('q')); ?>" class="w-full bg-slate-900 border border-slate-700 rounded-full py-2 pl-4 pr-10 text-sm text-white focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all outline-none">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="<?php echo e(route('wishlist.index')); ?>" class="w-10 h-10 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-800 hover:text-rose-400 transition-colors relative" title="Wishlist">
                            <i class="fa-regular fa-heart text-lg"></i>
                        </a>
                        <a href="<?php echo e(route('cart.index')); ?>" class="w-10 h-10 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-800 hover:text-emerald-400 transition-colors relative group" title="Keranjang">
                            <i class="fa-solid fa-shopping-cart text-lg"></i>
                            <span id="cartCount" class="absolute 0 top-0 right-0 w-4 h-4 bg-emerald-500 text-white text-[9px] font-bold flex items-center justify-center rounded-full border-2 border-slate-900 group-hover:border-slate-800 transition-colors"><?php if(auth()->guard()->check()): ?><?php echo e(\App\Models\Cart::where('user_id',Auth::id())->sum('quantity')); ?><?php else: ?> 0 <?php endif; ?></span>
                        </a>
                    </div>
                </div>
            </div>
    ...
    <footer class="bg-slate-950 pt-16 pb-8 border-t border-slate-800/50 mt-12">
        <div class="max-w-7xl mx-auto px-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12 mb-12">
                <!-- Col 1 -->
                <div>
                    <a href="<?php echo e(route('home')); ?>" class="text-3xl font-bold tracking-tighter text-white block mb-4">
                        SAL<span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-emerald-400">ZA</span>
                    </a>
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        ...
                        <li><a href="<?php echo e(route('shop')); ?>" class="text-sm text-slate-400 hover:text-purple-400 transition-colors flex items-center"><i class="fa-solid fa-angle-right text-[10px] mr-2"></i> Semua Produk</a></li>
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        <?php if(auth()->guard()->check()): ?>
                            <li><a href="<?php echo e(route('order.index')); ?>" class="text-sm text-slate-400 hover:text-emerald-400 transition-colors flex items-center"><i class="fa-solid fa-angle-right text-[10px] mr-2"></i> Pesanan Saya</a></li>
                        <?php else: ?>
                            <li><a href="<?php echo e(route('login')); ?>" class="text-sm text-slate-400 hover:text-emerald-400 transition-colors flex items-center"><i class="fa-solid fa-angle-right text-[10px] mr-2"></i> Akun Saya</a></li>
                        <?php endif; ?>
            ...
            <div class="pt-8 border-t border-slate-800/50 flex flex-col md:flex-row justify-between items-center gap-4">
                <p class="text-xs text-slate-500">&copy; <?php echo e(date('Y')); ?> SALZA. All Rights Reserved.</p>
    ...
    async function trackOrder() {
        ...
        try {
            const res = await fetch('<?php echo e(route("order.track")); ?>', {
                method: 'POST',
...
<?php /**PATH /home/irawan/laravel/shoesmarket/UMKMmm/UMKM/resources/views/layouts/app.blade.php ENDPATH**/ ?>